...
if ~isfield(opts,'sigvl');  opts.sigvl = 1; end
if ~isfield(opts,'sigvm');  opts.sigvm = 1; end
if ~isfield(opts,'sigvu');  opts.sigvu = 1; end


if ~isfield(opts,'sigx1l');  opts.sigx1l = 1; end
if ~isfield(opts,'sigx1m');  opts.sigx1m = 1; end
if ~isfield(opts,'sigx1u');  opts.sigx1u = 1; end
if ~isfield(opts,'sigx2l');  opts.sigx2l = 1; end
if ~isfield(opts,'sigx2m');  opts.sigx2m = 1; end
if ~isfield(opts,'sigx2u');  opts.sigx2u = 1; end
if ~isfield(opts,'sigx3l');  opts.sigx3l = 1; end
if ~isfield(opts,'sigx3m');  opts.sigx3m = 1; end
if ~isfield(opts,'sigx3u');  opts.sigx3u = 1; end
if ~isfield(opts,'sigx4l');  opts.sigx4l = 1; end
if ~isfield(opts,'sigx4m');  opts.sigx4m = 1; end
if ~isfield(opts,'sigx4u');  opts.sigx4u = 1; end
...
if ~isfield(opts,'adaplambda');  opts.adaplambda = 1; end
if ~isfield(opts,'maxiter');  opts.maxiter = 1e4; end
if ~isfield(opts,'maxtime');  opts.maxtime = 1e4; end
...
if ~isempty(At)
    ...
    elseif norm(lb-ub) == 0
        ...
        params.borg = b;
        params.Corg = C;
        if params.fflag == 0 && isempty(l) && isempty(u)
...
elseif isa(Bt,'double')
    ...
    params.B = Bt';
elseif  isa(Bt,'cell')
    params.Bmap = @(X) Bmap(X, Bt);
    params.BTmap = @(X) BTmap(X, Bt);
...
for iter = 1:opts.maxits
    ...
    for k = 1:params.nblock
        ...
        if params.Aboxflag == 1 && params.Axeqb == 0
            params.NEWT.Fd = params.NEWT.Fd - dot_ssn(Fnew.Fx1, out.dx1);
        end
        if params.fnonsmooth
    ...
    if (max([params.hists.pinf(end),params.hists.K1,params.hists.K2,params.hists.P1, params.hists.dinf(end)]) < opts.tol) && iter > 10
        if opts.tolscale < 100
            [ Xnew2, ynew2, Znew2,rec] = recover_var_chk2nd(y,Z,R,V,S,X4,params,iter);
            if max([rec.pinf,rec.dinf]) < opts.tol
                cstop = 1;
                y = ynew2;  Z= Znew2;
                X= Xnew2;
                params.rec = rec;
    ...
    if cstop
        out.status = sprintf('max(pinf,dinf) < %3.2e', opts.tol);
        break;
    end

    if (iter == maxits) || toc(params.tic) > opts.maxtime
...
if params.fflag && params.fnonsmooth
    ...
    for p =1:params.nblock
        if isfield(params.f{p},'shift') && iscell(tmp)
            tmp{p} = tmp{p} - params.f{p}.shift ;
        elseif isfield(params.f{p},'shift') && ~iscell(tmp)
...
function [X, y,  Z,rec] = recover_var_chk2nd(y,Z,R,V,S,X4,params,iter)
trCX = full(dot_ssn(full(params.C), X4.var));
QX4 = params.Qcmap(X4.var);
Qv = params.Qcmap(V.var);
xQx = 0.5*dot_ssn(QX4,X4.var);
