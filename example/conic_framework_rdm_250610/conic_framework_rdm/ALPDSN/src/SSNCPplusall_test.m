function [X, out, y, S, Zb] = SSNCPplusall_test(model, opts)
...
% AAtC = sparse([eye(length(b)) AC;AC' CC]);
...
%  [At,C,b,normA,normC,normb,X.var,y.var,S.var] = scaling(trans.blk,At,C,b,X.var,y.var,S.var);
...
trans.res = 1;
trans.hists.Fnorm = [];
trans.proj_index = opts.project_index;
%% Compute projection
if trans.isL
...
if trans.isL
    for iter = 1:maxits
        ...
        trans.NEWT.tau1 = 1*trans.NEWT.lambda*(F.FYres^trans.NEWT.sigPowy);
        trans.NEWT.tau2 = 1*trans.NEWT.lambda*(F.FZres^trans.NEWT.sigPowzb);
        trans.NEWT.tau3 = 1*trans.NEWT.lambda*(F.FXres^trans.NEWT.sigPowx);
        trans.NEWT.tau4 = 1*trans.NEWT.lambda*(F.Fqres)^trans.NEWT.sigPowq + 1e-7;
        ...
        for k = 1:trans.nblock
            ...
            trans.NEWT.Fd = trans.NEWT.Fd - sum(sum(Fnew.FZ{k}.* dzb));
        ...
        % condition on performing a newton step


        if Fnew.res < 5 * F.res || count <= 5 %5e1*F.res
            count = 0;
            y = ynew;
            F = Fnew;
            X = Xnew;
        ...
        if (max(trans.hists.pinforg(end), trans.hists.dinforg(end) * tolscale) < tol * retol)
            ...
            if opts.tolscale < 100
                ...
                if max([rec.pinf,rec.dinf*tolscale]) < tol*retol
                    cstop = 1;
    ...
    for iter = 1:maxits
        ...
        trans.iter = iter;
        %         F.FXres = 0;
        %         for p = 1:length(F.FX); F.FXres = F.FXres + sum(sum(F.FX{p}.*F.FX{p}))+1e-6; end;
        ...
        trans.NEWT.tau1 = 1*trans.NEWT.lambda*(F.FYres^trans.NEWT.sigPowy);
        trans.NEWT.tau2 = 1*trans.NEWT.lambda*(F.FXres^trans.NEWT.sigPowx);
        %         trans.NEWT.tau1 = 1e-6;
        %         trans.NEWT.tau2 = 1e-6;
        %         trans.NEWT.sig = 1 ;
...
%         nrmxxbest = norm(X.var-opts.X);
%         nrmyybest = norm(y.var-opts.y);


        [dx,dy,trans] = gendirectionxy2(F, K, At, trans, cgopts, opts.method);
        1;

        %         F2 = F;
        %         F2.FY = F2.FY - trans.NEWT.tau1*dy;
        %         F2.FX = F2.FX - trans.NEWT.tau2*dx;
        %
        %         [dx,dy,trans] = gendirectionxy(F2, K, At, trans, cgopts, opts.method);

        %         dx = -0.1*F.FX;
        %         dy = -0.1*F.FY;
        %         trans.cgres = [0 0];
        %         trans.cgiter = 2;
        %         trans.newton_res = 1e-6;
...
%         trans.mu = min(0.1,Fnew.res)^2*1e-4;
        ...
        if Fnew.res < F.res || count <= 5 %5e1*FZ.res
            recount = count;
            count = 0;
            y = ynew;
            F = Fnew;
        ...
        if (max(trans.hists.pinforg(end),trans.hists.dinforg(end)*tolscale) < tol*retol) || max([trans.hists.pinforg(end),trans.hists.dinforg(end),trans.hists.gaporg(end)]) < 1e-2
            if opts.tolscale < 100
...
%         if strcmp(opts.method, 'iterative')
%            trans.cgres(end) = trans.cgres(end)/1e13;
%            trans.newton_res = trans.newton_res/1e13;
%         end
        ...
        log_info = {'%5s', 'iter', num2str(iter);
            ...
            '%7s', 'pinforg', num2str(trans.hists.pinforg(end), '%2.1e');
            '%7s', 'dinforg', num2str(trans.hists.dinforg(end), '%2.1e');
            '%7s', 'FY', num2str(F.FYres, '%2.1e');
            '%7s', 'FX', num2str(F.FXres, '%2.1e');
            '%10s', 'F', num2str(F.res, '%5.5e');
%             '%10s', 'distx', num2str(trans.nrmyybest, '%5.5e');
%             '%10s', 'dyratio', num2str(trans.dyratio, '%2.1e');
%             '%10s', 'yratio', num2str(trans.yratio, '%2.1e');
            '%7s', 'nrmdx', num2str(nrmdx, '%2.1e');
            '%7s', 'nrmdy', num2str(nrmdy, '%2.1e');            
            '%7s', 'tau1', num2str(trans.NEWT.tau1, '%2.1e');
            '%7s', 'tau2', num2str(trans.NEWT.tau2, '%2.1e');
            '%7s', 'CGres', num2str(trans.cgres(end), '%2.1e');
            '%7s', 'nt_res', num2str(trans.newton_res, '%2.1e');
            '%5s', 'CGit', num2str(trans.cgiter, '%5d');
            '%7s', 'rank', num2str(length(Fnew.par.posidx{1}), '%3d');
            '%5s', 'count', num2str(recount, '%5d');
            % '%7s', 'ratio', num2str(trans.NEWT.ratio, '%2.1e')
            };
...
if trans.res < 0.01
[S0, F.par] = project_test(K, W ,[],[] ,trans.proj_index);
else
[S0, F.par] = project_test(K, W ,[],[] ,0);
end
...
if trans.res < 1e-3
for k = 1:trans.nblock
    F.Dh{k} = double(tmp2{k}> trans.xL{k}+ 0.5  & tmp2{k} < trans.xU{k} - 0.5 );
end
else
    for k = 1:trans.nblock
...
trans.hists.Fnorm = [trans.hists.Fnorm F.res];
...
if  trans.project_option
    X.var = project_test(K, X.var,[],[],trans.proj_index);
end

% if trans.NEWT.iter ==10
% fff = y.var'*b;
% bb = [b;fff];
% tmpc = trans.AmapC(X.var,bb);
...
mu = trans.mu;
[S0, F.par] = projectbarrier(K, W, mu);
end

% X.var = 0.999*trans.sigma * S0 + 0.001 * X.var ;
...
dobj = trans.scale.objscale * bTy;
pobj = trans.scale.objscale * trCX;
gap = abs(trCX - bTy) / (1 + abs(trCX) + abs(bTy)) ;
gaporg = abs(pobj - dobj) / (1+ abs(pobj) + abs(dobj));
...
y = y.var;
% trans.nrmyybest = norm(y - trans.y);
% if trans.iter > 1
% % tmpLchol =  trans.Lchol;
% 
% q(trans.Lchol.perm,1) = trans.Lchol.Rt*trans.y;
% ytmp = 1./trans.scale.Cscale * (sparse(diag(1./diag(trans.scale.DA))) * q);
% ytmp = y - ytmp;
% trans.yratio = norm(trans.eigve*ytmp)/norm(ytmp);
% trans.nrmyybest = norm(y - trans.y);
% else
% %     trans.nrmyybest = norm(y - trans.y);
% trans.yratio = 0;
...
trans.hists.pobj(iter + 1) = rec.pobj;
trans.hists.dobj(iter + 1) = rec.dobj;
trans.hists.pinf(iter + 1) = rec.pinf;
trans.hists.pinforg(iter + 1) = rec.pinf;
trans.hists.dinf(iter + 1) = rec.dinf;
trans.hists.dinforg(iter + 1) = rec.dinf;
trans.hists.gap(iter + 1) = rec.gap;
trans.hists.gaporg(iter + 1) = rec.gap;
trans.hists.pvd(iter + 1) = rec.pinf / rec.dinf;
trans.hists.dvp(iter + 1) = rec.dinf / rec.pinf;
trans.hists.pvdorg(iter + 1) = rec.pinf / rec.dinf;
trans.hists.dvporg(iter + 1) = rec.dinf / rec.pinf;
