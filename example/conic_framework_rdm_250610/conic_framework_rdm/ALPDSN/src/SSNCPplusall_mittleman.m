function [X, out, y, S, Zb] = SSNCPplusall_mittleman(model, opts)
...
for ii = 1:kk
    ...
    for jj = 1 : length(modeltmp.K{ii}.size)
        ...
        if strcmp(modeltmp.K{ii}.type,'s')
            n = size(tmpC2(cumsum_size(jj)+1:cumsum_size(jj+1),cumsum_size(jj)+1:cumsum_size(jj+1)),1);
            m = size(Attmp(sqcumsum_size(jj)+1:sqcumsum_size(jj+1),:),2);
            model2.C{lenk+jj,1} =  tmpC2(cumsum_size(jj)+1:cumsum_size(jj+1),cumsum_size(jj)+1:cumsum_size(jj+1));
            %                         model.C{lenk+jj,1} = para* C{1} ;
            model2.At{lenk+jj,1} = Attmp(sqcumsum_size(jj)+1:sqcumsum_size(jj+1),:);
            %                         model.At{lenk+jj,1} = para* At{1};
            %                         btmp = btmp + b;
            model2.K{lenk+jj,1} = cone2;
        else
            model2.C{lenk+jj,1} = tmpC2;
            model2.At{lenk+jj,1} = Attmp ;
            %                         model.At{lenk+jj,1} =   para*randn(size(Attmp));
            model2.K{lenk+jj,1} = cone2;
        end
    end
...
if trans.isL
    ...
    for iter = 1:maxits
        ...
        trans.NEWT.tau1 = 10*trans.NEWT.lambda*(F.FYres^trans.NEWT.sigPowy);
        trans.NEWT.tau2 = 10*trans.NEWT.lambda*(F.FXres^trans.NEWT.sigPowx);
...
%         trans.mu = min(0.1,Fnew.res)^2*1e-4;
        ...
        if Fnew.res < 50*F.res || count <= 5 %5e1*FZ.res
            recount = count;
            count = 0;
            y = ynew;
            F = Fnew;
