...
function [Y, par] = projectbarrier(K, X,mu, L, U)
    ...
    for p = 1:length(K)
        ...
        if strcmp(cone.type, 's')
            n = sum(cone.size);
            cumsum_size = [1 cumsum(cone.size)] ;
            if nargout == 1

%                 if (length(cone.size) ==  1) || (sum(cone.size) < 100)
                    [Yp] = project_sdp_barrier(X{p},mu, tol);
            else
%                 if (length(cone.size) == 1) || (sum(cone.size) < 100)
                    [Yp, P, Dsch2, dd, posidx] = project_sdp_barrier(X{p},mu, tol);
...
function [Y, V, Dsch2, d, posidx] = project_sdp_barrier(X, mu,tol)
    n = length(X);
...
exist_mexeig = 0;
    ...
    d2 = (d + sqrt(d.^2 + 4*mu))/2;
    
    for i = 1:n;
        for j = 1:i
            if d(i)~= d(j)
            Dsch2(i,j) = (d2(i) - d2(j)) / (d(i) - d(j));
            Dsch2(j,i) = Dsch2(i,j);
            else
            Dsch2(i,j) = 0.5 + d(i)/(sqrt(d(i)^2 +  4*mu))/2;
            Dsch2(j,i) = Dsch2(i,j);
            end
        end
    end
%     idx = idx(n:-1:1); %d = d(n:-1:1);
    ...
    Y = Vtmp * Vtmp';
    Y = 0.5 * (Y + Y');
%     Dsch22 = (dp * ones(1, s)) ./ (dp * ones(1, s) + ones(r, 1) * dn');
