...
function [Y, par] = project_test(K, X, L, U,option)
...
%                 if (length(cone.size) ==  1) || (sum(cone.size) < 100)
                    [Yp] = project_sdp(X{p}, tol,option);
%                 else
%                     [Yp] = project_sdp(X{p}, tol);
%                 end
            else
%                 if (length(cone.size) == 1) || (sum(cone.size) < 100)
                    [Yp, P, Dsch2, dd, posidx] = project_sdp(X{p}, tol,option);
%                 else
...
function [Y, V, Dsch2, d, posidx] = project_sdp(X, tol,option)
    n = length(X);
