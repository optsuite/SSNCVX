...
%       Aq = feval(matvecfname, K, At, par, x, AL);   
      Aq = feval(matvecfname,{K{1}.type, K{1}.size},At,par,x,par.Dsch1,AL);   
   else
      Aq = zeros(N,1);  
   end
   r = b-Aq;  
...
%        Aq = matvec_y(K, At, par, q, AL);
       Aq = feval(matvecfname,{K{1}.type, K{1}.size},At,par,q,par.Dsch1,AL); 
       sigma = q'*Aq; 
       if (abs(sigma) < tiny)
          solve_ok = 2; 
...
%%*************************************************************************
   ...
   function  q = precondfun(K, At,par,L,r)
