...
%
function [X, out, y, S, Zb] = SSNCPplusall_mittlemanbest(model, opts)
...
%         toll = 1;
%         while Fnew.res > F.res && toll < 2
%             Xnew.var = X.var + 0.8 ^ toll * dx ;
%             ynew.var = y.var + 0.8 ^ toll * dy;
%             toll = toll +1;
%             [Fnew,S,Xnew,trans] = compute_projection(K,Xnew,ynew,b,C,trans);
%         end
...
%     if iter < 300
%         avg_dvp = smean(trans.hists.dvp(sitr:iter));
% %         avg_dvp = smean(trans.hists.pvd(sitr:iter));
%     else
%         avg_dvp = smean(trans.hists.pvd(sitr:iter));
%         muNEWT.mu_fact = 1.2;
%         muNEWT.mu_delta = 5;
