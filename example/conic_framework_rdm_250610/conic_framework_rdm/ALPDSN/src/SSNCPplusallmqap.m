...
%
function [X, out, y, S, Zb] = SSNCPplusallmqap(model, opts)
...
if trans.isL
    for iter = 1:maxits
        ...
        trans.NEWT.tau1 = 0.1*trans.NEWT.lambda*(F.FYres^trans.NEWT.sigPowy);
        trans.NEWT.tau2 = 0.1*trans.NEWT.lambda*(F.FZres^trans.NEWT.sigPowzb);
        trans.NEWT.tau3 = trans.NEWT.lambda*(F.FXres^trans.NEWT.sigPowx) + 1e-7;
...
%         trans.NEWT.nrmd = sqrt(norm(dx)^2 + norm(dy)^2 + norm(dzb)^2 + norm(dq)^2);
        ...
        if Fnew.res < 50* F.res && count <= 5 %5e1*F.res
            count = 0;
            y = ynew;
            F = Fnew;
            X = Xnew;
...
%         if Fnew.res < 10* F.res && count <= 5 %5e1*F.res
%             count = 0;
%             y = ynew;
%             F = Fnew;
%             X = Xnew;
...
%         if iter >=2 && trans.hists.dinforg(end)*trans.hists.pinforg(end) <=  0.5*trans.hists.dinforg(end-1)*trans.hists.pinforg(end-1)
%             trans.NEWT.lambda = max(NEWTopts.gamma1 * trans.NEWT.lambda, 1e-16); 
%         elseif iter >=2 && trans.hists.dinforg(end)*trans.hists.pinforg(end) <= 2* trans.hists.dinforg(end-1)*trans.hists.pinforg(end-1)
%             trans.NEWT.lambda = NEWTopts.gamma2 * trans.NEWT.lambda;
%         else
%             trans.NEWT.lambda = NEWTopts.gamma3 * trans.NEWT.lambda;
%         end
% trans.hists.dinforg(end)
    ...
    for iter = 1:maxits
        ...
        if Fnew.res < 5e0*F.res && count <= 5 %5e1*FZ.res
            count = 0;
            y = ynew;
            F = Fnew;
            X = Xnew;
