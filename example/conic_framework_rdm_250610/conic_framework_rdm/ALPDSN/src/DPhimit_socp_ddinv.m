...
function Y = DPhimit_socp_ddinv(K, par, H, sig, Lambda1, vec_c)
...
%                 Y{p}(cumsumlen(jj)+1:cumsumlen(jj+1),cumsumlen(jj)+1:cumsumlen(jj+1))  = par.Dsch1{sumlen+jj}*H{p}(cumsumlen(jj)+1:cumsumlen(jj+1),cumsumlen(jj)+1:cumsumlen(jj+1));
    ...
    elseif strcmp(cone.type,'q')
        ...
        h = sig * H{p} ./ Lambda1;
        tmph = (Q3' * h) .* vec_c;
        tmph = repelem(tmph, cone.size);
        tmph = h + par.P3{p} .* tmph ./ Lambda1;

        tmp1 = Q3'*tmph.*par.Dsch3{p};
        tmp1 = repelem(tmp1,cone.size,1).*par.P3{p};
        tmp3 = par.shift3{p}.*tmph;
        Y{p} = tmp1+tmp3;
