...
function [Y, par] = projectmit2smooth(K, X, mu, socp_formula, L, U)
...
%             Yp = sparse(nsize,nsize);
    ...
    elseif strcmp(cone.type, 'q')
        if nargout == 1
            Yp = mexprox_cone_q(full(X{p}), mu, cone.size); 
        else
            if socp_formula == 1 || socp_formula == 2
                [Yp, dd, Dsch1, Dsch2, P1, P2, shift] = mexprox_cone_q(full(X{p}), mu, cone.size);
            ...
            if socp_formula == 2 || socp_formula == 3
                [Yp, DXdiag, DXcoeff, DXrank1] = mexprox_cone_q2(full(X{p}), mu, cone.size);
                par.shift3{p} = DXdiag;
                par.Dsch3{p} = DXcoeff;
                par.P3{p} = DXrank1;
            end

            par.socp_formula = socp_formula;
        end

    elseif strcmp(cone.type, 'l') || strcmp(cone.type, 'b2l')
        n = sum(cone.size);
        X_temp = sqrt(X{p}.^2 + 2 * mu);
        Yp = (X{p} + X_temp) / 2;

        if nargout > 1
            P1 = []; P2 = [];
            Dsch2 = 1 / 2 * (X{p} ./ X_temp + 1);
            Dsch1 = [];
