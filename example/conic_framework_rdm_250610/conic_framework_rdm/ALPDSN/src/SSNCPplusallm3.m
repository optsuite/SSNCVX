...
%
function [X, out, y, S, Zb] = SSNCPplusallm3(model, opts)
...
trans.projectP = @(X) sdp_project_P(K, X, trans); % to be implemented
...
for iter = 1:maxits
    ...
    if cgopts.CG_adapt %更改CG的tol
        cgopts.CG_tol = max(min(0.1 * F.FYres, 0.1), cgopts.cgtolmin) * 0.01; % F.res
    end

    trans.NEWT.tau1 = trans.NEWT.lambda*(F.FYres^trans.NEWT.sigPowy);
    trans.NEWT.tau2 = trans.NEWT.lambda*(F.FZres^trans.NEWT.sigPowzb);
    trans.NEWT.tau3 = trans.NEWT.lambda*(F.FXres^trans.NEWT.sigPowx);
    trans.NEWT.tau4 = trans.NEWT.lambda*(F.Fqres)^trans.NEWT.sigPowq + 1e-7;
    if iter == 1
        trans.NEWT.tau4 = 1;
    end
...
%     tmpnrm = norm(ynew.Avar + Snew.var + Zbnew.var - C);
    ...
    if (trans.NEWT.iter == 1) && (Fnew.res >= F.res)
    ...
    if Fnew.res < 5e1 * F.res || count <= 5 %5e1*F.res
        ...
        q = qnew;
        if iter > 2
            trans = sigPow_CGmaxit_updateplus(F.res,trans);
    ...
    if (max(trans.hists.pinforg(end), trans.hists.dinforg(end) * tolscale) < tol * retol)
        ...
        if opts.tolscale < 100
            [Xnew,ynew,Snew,Zbnew,rec] =recover_var_chk2ndplus(Zb, F, X, y, S, W, trans);
            ...
            cstop = 1;
            [Xnew,ynew,Snew,Zbnew,rec] = recover_var_chk2ndplus(Z,F,X,y,S,W,trans);
...
%     %         trans.NEWT.sig = 1 ;
%     if cgopts.CG_adapt %更改CG的tol
%         cgopts.CG_tol = max(min(0.1 * F.res, 0.1), cgopts.cgtolmin) * 0.1; % F.res
%     end
%     
...
%     [dx,dy,trans] =gendirectionxyori(F,{K{1}.type, K{1}.size},At,trans,cgopts);
...
%     [FZnew, S,trans] = compute_projection2({K{1}.type, K{1}.size},Xnew,ynew,b,C,trans);
%     
...
%         
%         if trans.pmu ~= pmup
%             tmpstr = ['-- sigma updated:', num2str(trans.pmu)];
%             if record >= 1; fprintf([tmpstr,'\n']); end
%             str = [str, tmpstr newline];par.Dsch1
%             %                 [F, X, Z] = cpmp_res_fp_updmu(X, Z, K, b, C, trans.pmu,pmup,trans);
%         end
%         
%         %             trans = mu_update_ADMM(trans,iter,muopts);
%     end
...
%     F.FYres = norm(F.FY);
   ...
   F.FYres = full(sqrt(sum(sum(F.FY.*F.FY))));
    ...
    %         trans.NEWT.sig = 1 ;
    if cgopts.CG_adapt %更改CG的tol
        cgopts.CG_tol = max(min(0.1*F.res, 0.1),cgopts.cgtolmin)*0.1;% FZ.res
    end
    
    %         [d,trans] = gendirection(FZ,blk,At,trans,cgopts,NEWTopts,scale_data);
%     [dx,dy,trans] = gendirectionxyori(F,blk,At,trans,cgopts);
    [dx,dy,trans] = gendirectionxy(F, K, At, trans, cgopts, opts.method);
    trans.cgall = trans.cgall + trans.cgiter;
    if iter == 20
        toc(trans.tic)
    ...
    [Fnew,S,trans] = compute_projection(K,Xnew,ynew,b,C,trans);
    ...
    if (max(trans.hists.pinforg(end),trans.hists.dinforg(end)*tolscale) < tol*retol)
        if opts.tolscale < 100
            [Xnew,ynew,Snew,rec] = recover_var_chk2nd(Z,F,X,y,S,W,trans);
            if max([rec.pinf,rec.dinf*tolscale,rec.K1/5,rec.K1dual,rec.C1]) < tol*retol
                cstop = 1;
                X = Xnew; y = ynew; S = Snew;
                trans.rec = rec;
            else
...
% %         end
% 
% %         fprintf(['%5d  %2.1e  %+2.1e  %+2.1e  %2.1e  %2.1e  %2.1e  %2.1e  ',...
% %             '%2.1e  %2.1e  %2.1e  %2.4e %2.4e %2.4e  %5d  %2.4e  %2.4e  %2.4e\n'],...
% %             iter, trans.pmu, trans.hists.pobj(end), trans.hists.dobj(end), ...
% %             trans.hists.gap(end), trans.hists.pinf(end), trans.hists.dinf(end),...
% %             trans.hists.gaporg(end), trans.hists.pinforg(end),trans.hists.dinforg(end),...
% %             FZ.res, trans.NEWT.tau1,trans.NEWT.tau2, trans.cgres(end), trans.cgiter, trans.NEWT.ratio,...
% %             thetaFZdorg, thetaFZFZnew);
%            fprintf(['%5d  %2.1e  %+2.1e  %+2.1e  %2.1e  %2.1e  ',...
%             '%2.1e  %2.1e  %2.1e  %2.4e %2.4e %2.4e  %5d  %2.4e  %2.4e  %2.4e\n'],...
%             iter, trans.pmu, trans.hists.pobj(end), trans.hists.dobj(end), ...
%             trans.hists.gap(end),...
%             trans.hists.gaporg(end), trans.hists.pinforg(end),trans.hists.dinforg(end),...
...
W = trans.ATmap(y.var) + 1 / trans.pmu * X.var - C + Zb.var;
[S0,F.par] = project(K,W);
F.par.precond = trans.precond; %加入预条件，为pre做准备

tmp2 = 1 / trans.pmu *q.var - Zb.var;
F.Dh = MatCell.zeros_like(C);
for k = 1:trans.nblock
    F.Dh{k} = tmp2{k}> 0;
end
tmp2 = trans.projectP(tmp2);
S.var = S0 - W;
S.Avar = trans.Amap(S.var);
F.FY = -b + trans.pmu*trans.Amap(S0);
F.FZ = trans.pmu * S0 - trans.pmu * tmp2;
F.FX = 1 / trans.pmu * X.var - S0;
F.Fq = 1 / trans.pmu * q.var - tmp2;
F.FYres = norm(F.FY);
F.FZres = norm(F.FZ);
F.FXres = norm(F.FX);
F.Fqres = norm(F.Fq);
...
PX = norm(trans.projectP(X.var) - X.var)/5;
PZb = norm(trans.projectP(- Zb.var))/5;
...
function [X, y, S, Z,rec] = recover_var_chk2ndplus(Z, F, X, y, S, W, trans)
...
function trans = sigPow_CGmaxit_updateplus(res,trans)
if res < 1e-4
    sigPowy      = trans.sigyl;
    sigPowx      =trans.sigxl;
...
function [dx,dy,trans]= gendirectionxyori(Ftz,blk,At,trans,cgopts,NEWTopts,scale_data)
nblock = trans.nblock;
Amap  = trans.Amap;
ATmap = trans.ATmap;
...
function [FZ,S,trans] = compute_projection2(blk,X,y,b,C,trans)
