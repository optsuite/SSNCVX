...
% %
% function [X, out, y, S] = SSNCPplus(model, opts)
...
% [F, X, trans] = comp_res_fp(Z, K, b, C, trans);
...
% %% Compute projection
% [F, S, trans] = compute_grad(K, X, y, Zb, b, C, trans);
...
%     trans.NEWT.tau2 = trans.NEWT.lambda*(F.Fbres^trans.NEWT.sigPowzb);
...
%     [Fnew,Snew,trans] = compute_grad(K,Xnew,ynew,Zbnew,qnew,b,C,trans);
...
%         trans.NEWT.Fd = trans.NEWT.Fd - sum(sum(Fnew.Fb{k}.*dzb{k}));
...
%     %------------------------------------------------------------------
%     % check optimality
%     trans = record_optimal_NEWT(C,b,trans,Z,F,X,y,S,Zb,iter);
    
    
%     if (max(trans.hists.pinforg(end), trans.hists.dinforg(end) * tolscale) < tol * retol)
        
%         if opts.tolscale < 100
%             [Xnew,ynew,Snew,Zbnew,rec] = recover_var_chk2nd(Z,F,X,y,S,W,Zb,trans);
...
%         else
%             cstop = 1;
%             [Xnew,ynew,Snew,rec] = recover_var_chk2nd(Z,F,X,y,S,W,Zb,trans);
...
% out = generate_outs(out,X,y,S,Zb,trans,opts,iter);
...
% b = b / bscale; %单位化
% C = 1 / Cscale * C;
% trans.xL = 1 / bscale * trans.xLorg;
% trans.xU = 1 / bscale * trans.xUorg;
...
% F.Fb = trans.pmu * S0 - trans.pmu2 * tmp2;
% F.FX = 1 / trans.pmu * X - S0;
% F.Fq = 1 / trans.pmu2 * q.var - tmp2;
% F.FYres = norm(F.FY);
% F.Fbres = norm(F.Fb);
% F.FXres = norm(F.FX);
% F.Fqres = norm(F.Fq);
% F.res = F.FYres + F.FXres + F.Fbres + F.Fqres;
% end
