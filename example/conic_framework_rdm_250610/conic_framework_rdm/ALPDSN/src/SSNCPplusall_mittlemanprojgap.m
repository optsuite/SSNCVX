function [X, out, y, S, Zb] = SSNCPplusall_mittlemanprojgap(model, opts)
...
%         F.res = F.FXres + F.FYres;
        ...
        if iter == 9
            mineig = 1;
            for k = 1:trans.nblock
                 mintmp = min(F.par.Dsch2{k},[],'all');
                 if mineig > mintmp 
                     mineig = mintmp;
                 end
            end
            trans.NEWT.tau1 = sqrt(mineig);
        end
...
%         end
        ...
        if (iter == maxits) || toc(trans.tic) > 40000
            out.status = sprintf('reach the maximum iteration');
            [Xnew,ynew,Snew,rec] = recover_var_chk2nd(Z,F,X,y,S,W,trans);
            X = Xnew; y = ynew; S = Snew;
            trans.rec = rec;
...
W = trans.ATmap(y.var) +  X.var / trans.pmu  - C + Zb.var;
[S0,F.par] = project(K,W);
F.par.precond = trans.precond; %加入预条件，为pre做准备

tmp2 = q.var   - Zb.var * trans.pmu;
F.Dh = MatCell.zeros_like(C);

for k = 1:trans.nblock
F.Dh{k} = double(tmp2{k}> trans.xL{k} & tmp2{k} < trans.xU{k} );
end
tmp2 = trans.projectP(tmp2)/trans.pmu;
% Zb.var = Zb.var - q.var/trans.pmu + tmp2; 
...
[S0, F.par] = project(K, W);
% X.var = 0.999*trans.pmu * S0 + 0.001 * X.var ;



% S0 = (X.var / trans.pmu + S0)/2;
...
F.FY = -b + trans.pmu * trans.Amap(S0);
% tmpFY = -b + trans.pmu * trans.Amap(S0);
% F.FY = tmpFY ;
% trans.FYtmp = F.FY;
...
F.FX = X.var / trans.pmu  - S0;
%  X.var = trans.pmu * S0 ;
% tmpFX = X.var / trans.pmu  - S0;
% F.FX = tmpFX ;
% trans.FXtmp = F.FX;

F.FYres = norm(F.FY);
F.FXres = norm(F.FX)+1e-6;
