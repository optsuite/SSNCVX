...
%
function [X, out, y, S, Zb] = SSNCPplusallm2(model, opts)
...
[F, S, trans] = compute_projection2({K{1}.type, K{1}.size}, X, y, b, C, trans);
end
...
AXorg = AXfun_sdpnal({K{1}.type, K{1}.size}, At, X); %原始的AX 在process_SDP函数那里A是经过除去对角的。
AX = fwsolve(Lchol, AXorg);
end

function Aty = Atymap(y, K, At, Lchol)
Aty = Atyfun_sdpnal({K{1}.type, K{1}.size}, At, bwsolve(Lchol, y));
end
...
% [S0, F.par] = project(K, W);
[S0, F.par] = project2_sdpnal({K{1}.type, K{1}.size},W);
F.par.Dsch2 = F.par.Dsch12;
% F.par.shift = {0};
F.par.precond = trans.precond; %加入预条件，为pre做准备
