...
function  [x,resnrm,solve_ok] = psqmr2(matvecfname,K,At,b,par,L,tol,maxit,AL) 
...
%%
%    if isfield(par,'P'); par.Pt = ops_sdpnal(par.P,'transpose'); end
%    if isfield(par,'P1'); par.P1t = ops_sdpnal(par.P1,'transpose'); end
%    if isfield(par,'P2'); par.P2t = ops_sdpnal(par.P2,'transpose'); end
%    if isfield(par,'Dsch12'); par.Dsch21 = ops_sdpnal(par.Dsch12,'transpose'); end
%%
   x = x0; 
   if (norm(x) > 0) 
