...
%
function [X, out, y, S, Zb] = SSNCPplusm(model, opts)
...
%% Compute projection
[F, S, trans] = compute_grad(K, X, y, Zb,q, b, C, trans);
...
%     tmpnrm = norm(ynew.Avar + Snew.var + Zbnew.var - C);
    ...
    trans = record_optimal_NEWT(C,b,trans,X,y,S,Zb,iter);


    if (max(trans.hists.pinforg(end), trans.hists.dinforg(end) * tolscale) < tol * retol)

        if opts.tolscale < 100
            [Xnew,ynew,Snew,Zbnew,rec] =recover_var_chk2nd(Zb, F, X, y, S, W, trans);
            ...
            cstop = 1;
            [Xnew,ynew,Snew,Zbnew,rec] = recover_var_chk2nd(Z,F,X,y,S,W,trans);
...
function trans = record_optimal_NEWT(C, b, trans, X, y, S, Zb, iter)
% check optimality
...
function [X, y, S, Z,rec] = recover_var_chk2nd(Z, F, X, y, S, W, trans)
