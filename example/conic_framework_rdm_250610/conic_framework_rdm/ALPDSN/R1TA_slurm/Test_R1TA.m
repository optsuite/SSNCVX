...
prorange = [49 1:48 50:file_len];
for gamma1 = [0.3 0.4 0.35 0.25 0.4 0.2 0.45]
%     for gamma2 = [0.9 0.95 0.8  0.85 0.75 0.7 0.71]
        for i = 3
            probname = probnames{i};
            model = data2model(dataset,probname,dir_data);
            save_path = char(strcat(save_root,'/reR1TAallx34mu_update_itr10mu_fact1.5gamma20.72gamma1'));
...
%             save_path = [save_path,'_',sgamma1,'_',sgamma2,'/'];
            save_path = [save_path,'_',sgamma1,'/'];
            ...
            opts.cgmax = 300; %
            opts.sigxl = 0.3; %
            opts.sigxm = 0.4;  %44 45 46 : 1(0.3 0.95)  53: 0.4 0.6(0.3 0.95) 54: 0.6 0.8(0.4 0.7)
            opts.sigxu = 0.4; %44 45 46 : 1(0.3 0.95)  52(0.4 0.6 0.4 0.7) 53: 0.4 0.6(0.3 0.95) 54: 0.6 0.8(0.4 0.7)
            opts.xfactor = 1;
            opts.gamma1 = gamma1;
            opts.gamma2 = 0.72;
            opts.gamma3 = 5;
            if i == 45 || i ==46 || i ==44
                opts.sigxl = 1; opts.sigxm = 1; opts.sigxu = 1;
            elseif i == 52 || i ==53
                opts.sigxl = 0.4; opts.sigxm = 0.6; opts.sigxu = 0.6;
            elseif i ==54
                opts.sigxl = 0.6; opts.sigxm = 0.8; opts.sigxu = 0.8;
            elseif i ==47
                opts.sigxl = 0.5; opts.sigxm = 0.8; opts.sigxu = 0.8;
            elseif i == 48
                opts.sigxl = 0.6; opts.sigxm = 1; opts.sigxu = 1;
            elseif i == 49
                opts.sigxl = 0.5; opts.sigxm = 1; opts.sigxu = 1;
            end
            opts.cgtol = 1e-10;
            ...
            if i ==43
                result_path = './result45.txt';
                fid = fopen(result_path,'a+');
                timegeo1 = geo_mean(timegeo);
                stimegeo = num2str(timegeo1);
                sgamma1 = num2str(opts.gamma1);
                sgamma2 = num2str(opts.gamma2);
                str = [newline,'前43个mu_update_itr10mu_fact1.5时间的几何平均值为:',stimegeo,',gamm1的值为',sgamma1,',gamm2的值为',sgamma2,newline];
                fprintf(fid,'%s',str);
                fprintf('\n')
                fprintf('前43个mu_update_itr10mu_fact1.5时间的几何平均值为:%.2e,gamm1的值为%.2e,gamm2的值为%.2e',timegeo1,opts.gamma1,opts.gamma2)
                fprintf('\n')
                timegeo1  = [];
            ...
            save(res_save_path, 'm', 'n', 'pobj', 'pinf', 'dinf', 'relgap', 'etaK1', 'etaK2', 'etaC1', 'etaC2', 'iter', 'time', 'X_rank', 'S_rank');
        ...
        str = [newline,'mu_update_itr10mu_fact1.5时间的几何平均值为:',stimegeo,',gamm1的值为',sgamma1,',gamm2的值为',sgamma2,newline];
        fprintf(fid,'%s',str);
        fprintf('\n')
        fprintf('mu_update_itr10mu_fact1.5时间的几何平均值为:%.2e,gamm1的值为%.2e,gamm2的值为%.2e',timegeo,opts.gamma1,opts.gamma2)
        fprintf('\n')
        timegeo  = [];
...
disp(newline);
fprintf('%s & %s & %s & %s & %s & %s & %s & %s & %s & %s & %s & %s & %s & %s & %s \\\\ \\hline \n', ...
    'probname', 'm', 'n', 'pobj', 'pinf', 'dinf', 'relgap', 'etaK1', 'etaK2', 'etaC1', 'etaC2', 'iter', 'time', 'X_rank', 'S_rank');
disp(table_str);
